const getDateSpread = (tasks) => {
    let minDate = tasks[0].startDate.getTime()
    let maxDate = tasks[0].endDate.getTime()
...
const trimInterval = (startDate, endDate, lowerBound, upperBound) => {
    ...
    if(startDate > upperBound) // hasn't started yet
        return -1

    if(endDate < lowerBound) // already finished
        return 1
    ...
    if(startDate >= lowerBound && endDate <= upperBound) { // included in interval
        return {
            startDate,
            endDate,
        }
    ...
    if(startDate < lowerBound)
        trimmedStartDate = lowerBound

    if(endDate > upperBound)
        trimmedEndDate = upperBound
...
const splitTasks = (tasks, n) => {
    ...
    for(let i = 0; i < n; i++) {
        ...
        for(let j = 0; j < tasks.length; j++) {
            ...
            result[i].push({
                ...currentTask,
                startDate: trimInterval(startDate, endDate, lowerBound, upperBound).startDate,
                endDate: trimInterval(startDate, endDate, lowerBound, upperBound).endDate,
